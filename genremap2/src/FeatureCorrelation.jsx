// FeatureCorrelation.jsx — Pearson correlation heatmap with genre filtering
...
export default function FeatureCorrelation({ songs }) {
  ...
  const featureNames = useMemo(() => {
    const first = filteredSongs.find(s => s.features && Object.keys(s.features).length > 0)
    return first ? Object.keys(first.features).sort() : []
  }, [filteredSongs])

  // Build feature vectors from filtered songs
  const featureVectors = useMemo(() => {
    const map = {}
    for (const name of featureNames) {
      map[name] = filteredSongs
        .filter(s => s.features?.[name] != null)
        .map(s => s.features[name])
    }
    return map
  }, [filteredSongs, featureNames])
  ...
  const genreColor = selectedGenre !== 'all' ? GENRE_COLORS[selectedGenre] : 'var(--accent)'
      ...
      <div className={styles.controls}>
        ...
        <div className={styles.controlsRight}>
          ...
          <div className={styles.genreFilter}>
            <span className={styles.controlLabel} style={{ marginRight: 4 }}>GENRE</span>
            <select
              className={styles.genreSelect}
              value={selectedGenre}
              onChange={(e) => setSelectedGenre(e.target.value)}
              style={{
                borderColor: selectedGenre !== 'all' ? genreColor : 'var(--border)',
                color: selectedGenre !== 'all' ? genreColor : 'var(--text-primary)'
              }}
            >
              {genres.map(g => (
      ...
      <div className={styles.infoBar}>
        {hoveredCell ? (
          ...
          <span className={styles.infoHint}>
            {selectedGenre === 'all' 
              ? 'Hover a cell to inspect correlation' 
              : `Correlations for ${selectedGenre} (${filteredSongs.length} songs)`}
          </span>
        )}
      </div>
