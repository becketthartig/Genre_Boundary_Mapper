...
const VIZ_INFO = {
  scatter: {
    title: 'Genre PCA Map',
    body: `This map uses Linear Discriminant Analysis (LDA) — a supervised dimensionality reduction technique — to project all 57 audio features down to 2 dimensions. Unlike PCA, LDA specifically tries to maximize separation between the known genre labels, so genres that are acoustically distinct should form clear clusters.

Each point is a song. The axes (LD1 and LD2) are linear combinations of the 57 features that best separate genres. Points close together share similar acoustic characteristics within the LDA feature space.

The boundary filter on the right lets you show only songs near genre boundaries — songs that are acoustically ambiguous and hard to classify.`,
  },
  violin: {
