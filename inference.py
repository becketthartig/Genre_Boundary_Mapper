"""
inference.py
------------
Handles new song uploads: normalize → assign cluster → project to 2D.

This loads the pre-fitted pipeline.pkl so new songs land in the same
coordinate space as the existing dataset without re-fitting anything.

Usage (standalone test):
  python inference.py --wav my_new_song.wav --pipeline pipeline.pkl

Used by FastAPI:
  from inference import load_pipeline, process_new_song
"""

import json
import pickle
import argparse
import numpy as np
from pathlib import Path


def load_pipeline(pkl_path: str = "pipeline.pkl") -> dict:
    """
    Load the fitted pipeline from disk.
    Call this once at FastAPI startup and cache the result.

    Returns dict with keys: scaler, cluster_model, reducer, feat_cols
    """
    with open(pkl_path, "rb") as f:
        pipeline = pickle.load(f)

    required_keys = {"scaler", "cluster_model", "reducer", "feat_cols"}
    if not required_keys.issubset(pipeline.keys()):
        raise ValueError(f"pipeline.pkl is missing keys: {required_keys - pipeline.keys()}")

    print(f"Pipeline loaded: {len(pipeline['feat_cols'])} features, "
          f"{pipeline['cluster_model'].n_clusters} clusters")
    return pipeline


def process_new_song(
    filename: str,
    feature_list: list[float],
    pipeline: dict,
) -> dict:
    """
    Process a single new song through the fitted pipeline.

    Args:
        filename     - original filename, used as display label
        feature_list - output of extractor.extract_features(wav_path)
                       must match the column order in the training CSV
        pipeline     - loaded pipeline dict from load_pipeline()

    Returns:
        dict with id, filename, cluster, x, y, features
        (same shape as the objects in results.json, ready to append)
    """
    scaler = pipeline["scaler"]
    cluster_model = pipeline["cluster_model"]
    reducer = pipeline["reducer"]
    feat_cols = pipeline["feat_cols"]

    # Validate feature count
    if len(feature_list) != len(feat_cols):
        raise ValueError(
            f"Feature count mismatch: extractor returned {len(feature_list)} features "
            f"but pipeline expects {len(feat_cols)}. "
            f"Make sure extract_features() returns columns in the same order as the training CSV."
        )

    # Build feature dict for the response
    features = dict(zip(feat_cols, feature_list))

    # Normalize using the fitted scaler (do NOT re-fit)
    X = np.array(feature_list, dtype=np.float32).reshape(1, -1)
    X_scaled = scaler.transform(X)

    # Assign to nearest cluster centroid
    cluster_id = int(cluster_model.predict(X_scaled)[0])

    # Project into the existing 2D space
    # Note: UMAP's transform() for a single point can be slightly noisy.
    # This is expected behavior — the point is placed relative to its neighbors.
    coords = reducer.transform(X_scaled)
    x, y = float(coords[0, 0]), float(coords[0, 1])

    return {
        "filename": filename,
        "true_label": "unknown",  # no ground truth for user uploads
        "cluster": cluster_id,
        "x": x,
        "y": y,
        "features": features,
    }


def append_to_results(
    new_song: dict,
    results_path: str = "results.json",
) -> list[dict]:
    """
    Add a newly processed song to the results JSON and save.
    Assigns a new id based on current max id.

    Returns the updated full list.
    """
    with open(results_path, "r") as f:
        songs = json.load(f)

    max_id = max(s["id"] for s in songs) if songs else -1
    new_song["id"] = max_id + 1
    songs.append(new_song)

    with open(results_path, "w") as f:
        json.dump(songs, f, indent=2)

    print(f"Added '{new_song['filename']}' as id={new_song['id']} → cluster {new_song['cluster']}")
    return songs


# ─── Standalone test ──────────────────────────────────────────────────────────

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--wav", required=True, help="Path to .wav file to process")
    parser.add_argument("--pipeline", default="pipeline.pkl")
    parser.add_argument("--results", default="results.json")
    args = parser.parse_args()

    # Import your extractor
    import sys
    sys.path.insert(0, ".")
    from extractor import extract_features

    print(f"Extracting features from {args.wav}...")
    feature_list = extract_features(args.wav)

    pipeline = load_pipeline(args.pipeline)
    result = process_new_song(
        filename=Path(args.wav).name,
        feature_list=feature_list,
        pipeline=pipeline,
    )

    print(f"\nResult:")
    print(f"  Cluster:     {result['cluster']}")
    print(f"  Position:    ({result['x']:.3f}, {result['y']:.3f})")
    print(f"  Features:    {len(result['features'])} extracted")

    append_to_results(result, args.results)
