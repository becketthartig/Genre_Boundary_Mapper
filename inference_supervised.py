...
def load_pipeline(pkl_path: str = "pipeline.pkl") -> dict:
    ...
    Returns dict with keys: scaler, model, feat_cols, method
    """
    ...
    if "model" not in pipeline:
        ...
        if "lda" in pipeline:
            pipeline["model"] = pipeline["lda"]
            pipeline["method"] = "lda"
        elif "pca" in pipeline:
            pipeline["model"] = pipeline["pca"]
            pipeline["method"] = "pca"
        elif "umap" in pipeline:
            pipeline["model"] = pipeline["umap"]
            pipeline["method"] = "umap"
        else:
            raise ValueError("Pipeline file is corrupted or in unknown format")

    required_keys = {"scaler", "model", "feat_cols"}
    if not required_keys.issubset(pipeline.keys()):
        raise ValueError(f"pipeline.pkl is missing keys: {required_keys - pipeline.keys()}")

    method = pipeline.get("method", "unknown")
    n_components = getattr(pipeline["model"], "n_components", 2)
