...
-----------------------
Handles new song uploads in supervised mode.

New songs are projected into the existing PCA space and their genre
is set to "unknown" since we don't have ground truth labels for uploads.
...
def load_pipeline(pkl_path: str = "pipeline.pkl") -> dict:
    ...
    Returns dict with keys: scaler, pca, feat_cols
    """
    with open(pkl_path, "rb") as f:
        pipeline = pickle.load(f)

    required_keys = {"scaler", "pca", "feat_cols"}
    ...
    print(f"Pipeline loaded: {len(pipeline['feat_cols'])} features, "
          f"PCA with {pipeline['pca'].n_components} components")
    return pipeline
...
def process_new_song(
    ...
    """
    Process a single new song through the fitted PCA pipeline.
    ...
    Returns:
        dict with id, filename, genre (unknown), x, y, features
    """
